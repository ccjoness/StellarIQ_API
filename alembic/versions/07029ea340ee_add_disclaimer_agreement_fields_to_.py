...
def upgrade() -> None:
    ...
    op.add_column('users', sa.Column('agreed_to_disclaimer', sa.Boolean(), nullable=False, server_default='false'))
    op.add_column('users', sa.Column('disclaimer_agreed_at', sa.DateTime(timezone=True), nullable=True))

    # Update existing users to have agreed to disclaimer for backward compatibility
    op.execute("UPDATE users SET agreed_to_disclaimer = true, disclaimer_agreed_at = created_at WHERE agreed_to_disclaimer = false")
