...
from app.schemas.analysis import (
    AnalysisThresholds, IndicatorSignal, MarketAnalysisResult,
    MarketCondition
)
...
class MarketAnalysisService:
    ...
    async def analyze_symbol(
        ...
        try:
            ...
            if asset_type == "stock":
                ...
                rate_data = await self.market_service.get_crypto_exchange_rate(symbol, "USD")
            ...
            try:
                ...
                bbands_signal = self._analyze_bollinger_bands(bbands_data, current_price)
                if bbands_signal:
            ...
            overall_condition, confidence_score, recommendation, risk_level = self._calculate_overall_analysis(signals)
    ...
    def _analyze_rsi(self, rsi_data: List[RSIData], thresholds: AnalysisThresholds) -> Optional[IndicatorSignal]:
        ...
        if current_rsi >= thresholds.rsi_overbought:
            strength = min((current_rsi - thresholds.rsi_overbought) / (100 - thresholds.rsi_overbought), 1.0)
            ...
            strength = min((thresholds.rsi_oversold - current_rsi) / thresholds.rsi_oversold, 1.0)
            return IndicatorSignal(
                indicator="RSI",
    ...
    def _analyze_macd(self, macd_data: List[MACDData]) -> Optional[IndicatorSignal]:
        ...
        if current.macd > current.macd_signal and previous.macd <= previous.macd_signal:
            # Bullish crossover
            strength = min(abs(current.macd - current.macd_signal) / abs(current.macd), 1.0)
            return IndicatorSignal(
                indicator="MACD",
            ...
            # Bearish crossover
            strength = min(abs(current.macd_signal - current.macd) / abs(current.macd), 1.0)
            return IndicatorSignal(
                indicator="MACD",
    ...
    def _analyze_stochastic(self, stoch_data: List[StochData], thresholds: AnalysisThresholds) -> Optional[IndicatorSignal]:
        ...
        if avg_stoch >= thresholds.stoch_overbought:
            strength = min((avg_stoch - thresholds.stoch_overbought) / (100 - thresholds.stoch_overbought), 1.0)
            return IndicatorSignal(
            ...
            strength = min((thresholds.stoch_oversold - avg_stoch) / thresholds.stoch_oversold, 1.0)
            return IndicatorSignal(
                indicator="Stochastic",
                condition=MarketCondition.OVERSOLD,
                value=avg_stoch,
    ...
    def _analyze_bollinger_bands(self, bbands_data: List[BollingerBandsData], current_price: Optional[float]) -> Optional[IndicatorSignal]:
    ...
    def _calculate_overall_analysis(self, signals: List[IndicatorSignal]) -> tuple[MarketCondition, float, str, str]:
