...
class CryptoPortfolioService:
    ...
    async def get_user_portfolio(self, user_id: int) -> CryptoPortfolioResponse:
        ...
        portfolio_items = (
            ...
            .filter(CryptoPortfolio.user_id == user_id)
            .all()
        )
        ...
        for item in portfolio_items:
            try:
                ...
                current_price = DataParser.parse_crypto_exchange_rate(
                    rate_data
                ).exchange_rate
                ...
                profit_loss = (
                    total_value_item - investment if item.average_buy_price else None
                )
                profit_loss_percentage = (
                    (profit_loss / investment * 100)
                    if investment > 0 and profit_loss is not None
                    else None
                )
    ...
    def add_portfolio_item(
        self, user_id: int, item_data: CryptoPortfolioCreate
    ) -> CryptoPortfolio:
        ...
        if existing_item:
            ...
            if item_data.average_buy_price and existing_item.average_buy_price:
                ...
                total_investment = (
                    existing_item.amount * existing_item.average_buy_price
    ...
    def update_portfolio_item(
        self, user_id: int, symbol: str, update_data: CryptoPortfolioUpdate
    ) -> Optional[CryptoPortfolio]:
