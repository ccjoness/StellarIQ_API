...
class TechnicalAnalyzer:
    ...
    def parse_rsi_data(data: Dict[str, Any]) -> List[RSIData]:
        ...
        try:
            ...
            for timestamp, values in technical_analysis.items():
                result = RSIData(timestamp=timestamp, rsi=float(values.get("RSI", 0)))
    ...
    def analyze_rsi(rsi_data: List[RSIData]) -> tuple[MarketCondition, str]:
        ...
        if current_rsi >= 70:
            return (
                ...
                f"RSI at {current_rsi:.2f} indicates overbought conditions",
            )
        elif current_rsi <= 30:
            return (
                MarketCondition.OVERSOLD,
                f"RSI at {current_rsi:.2f} indicates oversold conditions",
            )
        else:
            return (
                MarketCondition.NEUTRAL,
                f"RSI at {current_rsi:.2f} indicates neutral conditions",
            )
    ...
    def analyze_stoch(stoch_data: List[StochData]) -> tuple[MarketCondition, str]:
        ...
        if current.slowk >= 80 and current.slowd >= 80:
            return (
                MarketCondition.OVERBOUGHT,
                f"Stochastic K={current.slowk:.2f}, D={current.slowd:.2f} indicates overbought",
            )
        elif current.slowk <= 20 and current.slowd <= 20:
            return (
                MarketCondition.OVERSOLD,
                f"Stochastic K={current.slowk:.2f}, D={current.slowd:.2f} indicates oversold",
            )
        else:
            return (
                MarketCondition.NEUTRAL,
                f"Stochastic K={current.slowk:.2f}, D={current.slowd:.2f} indicates neutral",
            )

    @staticmethod
    def analyze_bbands(
        bbands_data: List[BollingerBandsData], current_price: Optional[float] = None
        ...
        if current_price:
            if current_price >= current.real_upper_band:
                return (
                    MarketCondition.OVERBOUGHT,
                    f"Price {current_price:.2f} at upper band {current.real_upper_band:.2f} - overbought",
                )
            elif current_price <= current.real_lower_band:
                return (
                    MarketCondition.OVERSOLD,
                    f"Price {current_price:.2f} at lower band {current.real_lower_band:.2f} - oversold",
                )
            else:
                return (
                    MarketCondition.NEUTRAL,
                    f"Price {current_price:.2f} within bands - neutral",
                )
        else:
            return (
                MarketCondition.NEUTRAL,
                "Bollinger Bands available but no current price for comparison",
            )
