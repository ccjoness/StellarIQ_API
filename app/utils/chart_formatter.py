...
from app.schemas.crypto import CryptoTimeSeriesData
...
class ChartFormatter:
    ...
    def format_candlestick_data(
        ...
        for data_point in time_series_data:
            if isinstance(data_point, CryptoTimeSeriesData):
                # Handle crypto data
                candlestick = CandlestickData(
                    timestamp=data_point.timestamp,
                    open=data_point.open_usd,
                    high=data_point.high_usd,
                    low=data_point.low_usd,
                    close=data_point.close_usd,
                    volume=int(data_point.volume),  # Convert to int for consistency
                )
            else:
                # Handle stock data
                candlestick = CandlestickData(
    ...
    def format_rsi_overlay(rsi_data: List[RSIData]) -> List[LineChartData]:
        ...
        return [
            LineChartData(timestamp=data.timestamp, value=data.rsi) for data in rsi_data
        ]

    @staticmethod
    def format_macd_overlay(
        macd_data: List[MACDData],
    ) -> Dict[str, List[LineChartData]]:
        """Format MACD data for chart overlay."""
        return {
            "macd": [
                LineChartData(timestamp=data.timestamp, value=data.macd)
                for data in macd_data
            ],
            "macd_signal": [
                LineChartData(timestamp=data.timestamp, value=data.macd_signal)
                for data in macd_data
            ],
            "macd_histogram": [
                LineChartData(timestamp=data.timestamp, value=data.macd_hist)
                for data in macd_data
            ],
        }

    @staticmethod
    def format_stoch_overlay(
        stoch_data: List[StochData],
    ) -> Dict[str, List[LineChartData]]:
        """Format Stochastic data for chart overlay."""
        return {
            "stoch_k": [
                LineChartData(timestamp=data.timestamp, value=data.slowk)
                for data in stoch_data
            ],
            "stoch_d": [
                LineChartData(timestamp=data.timestamp, value=data.slowd)
                for data in stoch_data
            ],
        }

    @staticmethod
    def format_bbands_overlay(
        bbands_data: List[BollingerBandsData],
    ) -> Dict[str, List[LineChartData]]:
        """Format Bollinger Bands data for chart overlay."""
        return {
            "bb_upper": [
                LineChartData(timestamp=data.timestamp, value=data.real_upper_band)
                for data in bbands_data
            ],
            "bb_middle": [
                LineChartData(timestamp=data.timestamp, value=data.real_middle_band)
                for data in bbands_data
            ],
            "bb_lower": [
                LineChartData(timestamp=data.timestamp, value=data.real_lower_band)
                for data in bbands_data
            ],
        }
    ...
    def align_timestamps(
        candlestick_data: List[CandlestickData], indicator_data: List[LineChartData]
    ) -> List[LineChartData]:
        """Align indicator data timestamps with candlestick data."""
        candlestick_timestamps = {data.timestamp for data in candlestick_data}

        aligned_data = [
            data for data in indicator_data if data.timestamp in candlestick_timestamps
        ]
    ...
    def create_price_comparison_data(
        symbols_data: Dict[str, List[TimeSeriesData]], normalize: bool = False
        ...
        for timestamp in sorted_timestamps:
            ...
            for symbol, data in symbols_data.items():
                ...
                if price is not None:
                    if normalize and symbol in base_prices and base_prices[symbol] > 0:
                        ...
                        prices[symbol] = (
                            (price - base_prices[symbol]) / base_prices[symbol]
                        ) * 100
                    else:
                        prices[symbol] = price
            ...
            if prices:  # Only add if we have at least one price
                comparison_data.append(
                    PriceComparisonData(timestamp=timestamp, prices=prices)
                )
