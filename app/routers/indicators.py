...
from app.schemas.indicators import (
    RSIResponse, MACDResponse, StochResponse, BollingerBandsResponse,
    TechnicalAnalysisSummary, MarketCondition
)
...
async def get_rsi(
    ...
    try:
        ...
        data = await market_service.get_rsi(symbol.upper(), interval, time_period, series_type)
...
async def get_bollinger_bands(
    symbol: str,
    interval: Optional[str] = Query("daily", description="1min, 5min, 15min, 30min, 60min, daily, weekly, monthly"),
    time_period: Optional[int] = Query(20, description="Number of periods for Bollinger Bands calculation"),
    series_type: Optional[str] = Query("close", description="open, high, low, close"),
    current_user: User = Depends(get_current_active_user)
):
    ...
    try:
        market_service = MarketDataService()
        data = await market_service.get_bbands(symbol.upper(), interval, time_period, series_type)
