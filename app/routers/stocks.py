...
from app.schemas.stock import (
    StockQuote, StockSearchResponse, StockDailyResponse, 
    StockIntradayResponse, StockSearchResult
)
...
async def get_stock_intraday(
    symbol: str,
    interval: Optional[str] = Query("5min", description="1min, 5min, 15min, 30min, 60min"),
    outputsize: Optional[str] = Query("compact", description="compact or full"),
    current_user: User = Depends(get_current_active_user)
):
    ...
    try:
        ...
        market_service = MarketDataService()
        data = await market_service.get_stock_intraday(symbol.upper(), interval, outputsize)
