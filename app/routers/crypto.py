...
@router.get("/popular")
async def get_popular_cryptos(current_user: User = Depends(get_current_active_user)):
...
async def get_crypto_categories(current_user: User = Depends(get_current_active_user)):
...
async def get_crypto_exchange_rate(
    from_currency: str,
    to_currency: Optional[str] = Query(
        "USD", description="Target currency (default: USD)"
    ),
...
async def get_multiple_crypto_rates(
    ...
    try:
        ...
        return {"rates": rates, "to_currency": to_currency.upper(), "timestamp": "now"}
...
async def get_crypto_intraday(
    ...
    interval: Optional[str] = Query(
        "5min", description="Time interval (1min, 5min, 15min, 30min, 60min)"
    ),
    current_user: User = Depends(get_current_active_user),
):
    ...
    try:
        ...
        market_service = MarketDataService()
        data = await market_service.get_crypto_intraday(
            symbol.upper(), market.upper(), interval
        )
...
async def get_crypto_weekly(
    ...
    try:
        ...
        metadata = DataParser.get_crypto_metadata(data)
        time_series_data = DataParser.parse_crypto_daily_data(
            data
...
async def get_crypto_quote(
    ...
    try:
        market_service = MarketDataService()
        data = await market_service.get_crypto_exchange_rate(
            symbol.upper(), to_currency.upper()
        )
...
async def search_crypto(
    ...
    try:
        ...
        return CryptoSearchResponse(
            results=results[:20], total_count=len(results)  # Limit to 20 results
        )
...
@router.get("/trending")
async def get_trending_crypto(current_user: User = Depends(get_current_active_user)):
...
async def add_crypto_to_portfolio(
    ...
    try:
        ...
        logger.error(
            f"Error adding crypto to portfolio for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def update_crypto_in_portfolio(
    ...
    try:
        portfolio_service = CryptoPortfolioService(db)
        item = portfolio_service.update_portfolio_item(
            current_user.id, symbol, update_data
        )
        ...
        logger.error(
            f"Error updating crypto in portfolio for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def remove_crypto_from_portfolio(
    ...
    try:
        ...
        logger.error(
            f"Error removing crypto from portfolio for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
