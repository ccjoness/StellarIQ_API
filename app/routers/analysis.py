...
async def analyze_symbol(
    ...
    rsi_overbought: Optional[float] = Query(
        70.0, description="RSI overbought threshold"
    ),
    rsi_oversold: Optional[float] = Query(30.0, description="RSI oversold threshold"),
    stoch_overbought: Optional[float] = Query(
        80.0, description="Stochastic overbought threshold"
    ),
    stoch_oversold: Optional[float] = Query(
        20.0, description="Stochastic oversold threshold"
    ),
    ...
    try:
        ...
        result = await analysis_service.analyze_symbol(
            symbol.upper(), asset_type, thresholds
        )
...
async def bulk_analysis(
    request: BulkAnalysisRequest, current_user: User = Depends(get_current_active_user)
):
    ...
    try:
        ...
        for symbol in request.symbols:
            try:
                result = await analysis_service.analyze_symbol(
                    symbol.upper(), request.asset_type, request.thresholds
                )
        ...
        summary = {
            "overbought": sum(
                1 for r in results if r.overall_condition == MarketCondition.OVERBOUGHT
            ),
            "oversold": sum(
                1 for r in results if r.overall_condition == MarketCondition.OVERSOLD
            ),
            "neutral": sum(
                1 for r in results if r.overall_condition == MarketCondition.NEUTRAL
            ),
        }

        return BulkAnalysisResponse(
            results=results, summary=summary, analysis_timestamp=datetime.utcnow()
        )
...
async def analyze_watchlist(
    ...
    try:
        ...
        for favorite in favorites:
            try:
                asset_type = (
                    "stock" if favorite.asset_type == AssetType.STOCK else "crypto"
                )
                result = await analysis_service.analyze_symbol(
                    favorite.symbol, asset_type
                )
        ...
        overbought_count = sum(
            ...
            if r.overall_condition == MarketCondition.OVERBOUGHT
        )
        oversold_count = sum(
            1
            for r in analysis_results
            if r.overall_condition == MarketCondition.OVERSOLD
        )
        neutral_count = sum(
            1
            for r in analysis_results
            if r.overall_condition == MarketCondition.NEUTRAL
        )
...
async def market_screener(
    condition: MarketCondition = Query(
        ..., description="overbought, oversold, or neutral"
    ),
    min_confidence: Optional[float] = Query(
        0.6, description="Minimum confidence score"
    ),
    asset_type: Optional[str] = Query("stock", description="stock or crypto"),
