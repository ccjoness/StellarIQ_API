...
from app.schemas.charts import (
    ChartDataResponse, ComparisonChartResponse, MultiSymbolChartData
)
...
async def get_candlestick_chart(
    symbol: str,
    interval: Optional[str] = Query("daily", description="1min, 5min, 15min, 30min, 60min, daily, weekly, monthly"),
    ...
    indicators: Optional[str] = Query("rsi,macd,bbands", description="Comma-separated list of indicators"),
    ...
    try:
        ...
        # Get price data
        if interval == "daily":
            price_data = await market_service.get_stock_daily(symbol.upper(), outputsize)
            ...
            price_data = await market_service.get_stock_intraday(symbol.upper(), interval, outputsize)
            metadata = DataParser.get_metadata(price_data)
        ...
        if include_indicators and indicators:
            ...
            try:
                if "rsi" in indicator_list:
                    rsi_response = await market_service.get_rsi(symbol.upper(), interval)
                    rsi_data = TechnicalAnalyzer.parse_rsi_data(rsi_response)
                
                if "macd" in indicator_list:
                    macd_response = await market_service.get_macd(symbol.upper(), interval)
                    macd_data = TechnicalAnalyzer.parse_macd_data(macd_response)
                
                if "stoch" in indicator_list:
                    stoch_response = await market_service.get_stoch(symbol.upper(), interval)
                    stoch_data = TechnicalAnalyzer.parse_stoch_data(stoch_response)
                
                if "bbands" in indicator_list:
                    bbands_response = await market_service.get_bbands(symbol.upper(), interval)
...
async def get_comparison_chart(
    ...
    normalize: Optional[bool] = Query(False, description="Normalize to percentage change"),
    outputsize: Optional[str] = Query("compact", description="compact or full"),
    current_user: User = Depends(get_current_active_user)
):
    ...
    try:
        ...
        for symbol in symbol_list:
            try:
                if interval == "daily":
                    price_data = await market_service.get_stock_daily(symbol, outputsize)
                    time_series_data = DataParser.parse_daily_data(price_data)
                else:
        ...
        comparison_data = ChartFormatter.create_price_comparison_data(symbols_data, normalize)
...
async def get_multi_symbol_charts(
    symbols: str,
    interval: Optional[str] = Query("daily", description="daily, weekly, monthly"),
    include_indicators: Optional[bool] = Query(False, description="Include technical indicators"),
    current_user: User = Depends(get_current_active_user)
):
    ...
    try:
        ...
        for symbol in symbol_list:
            try:
                ...
                candlestick_data = ChartFormatter.format_candlestick_data(time_series_data)
                candlestick_data = ChartFormatter.limit_data_points(candlestick_data, 50)  # Fewer points for dashboard
                ...
                if include_indicators:
                    try:
                        ...
                        rsi_data = TechnicalAnalyzer.parse_rsi_data(rsi_response)
                        technical_overlay = ChartFormatter.create_technical_overlay(rsi_data=rsi_data)
                    except Exception as e:
