...
from app.schemas.favorite import (
    FavoriteCreate, FavoriteResponse, FavoritesListResponse, 
    FavoriteDelete, FavoriteWithQuote
)
...
async def get_favorites(
    include_quotes: Optional[bool] = Query(True, description="Include current market quotes"),
    ...
    try:
        ...
        if include_quotes:
            if asset_type:
                ...
                favorites = favorites_service.get_favorites_by_type(current_user.id, asset_type)
                ...
                favorites_with_quotes = await favorites_service.get_favorites_with_quotes(current_user.id)
        else:
        ...
        return FavoritesListResponse(
            favorites=favorites_with_quotes,
            total_count=len(favorites_with_quotes)
        )
...
async def get_favorites_stats(
    ...
    try:
        ...
        stock_count = len(favorites_service.get_favorites_by_type(current_user.id, AssetType.STOCK))
        crypto_count = len(favorites_service.get_favorites_by_type(current_user.id, AssetType.CRYPTO))
